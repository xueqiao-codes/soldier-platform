...
count=$( ps -ef | grep "dal_set_agent_daemon" | egrep jar | grep -v grep | wc -l )
if [ $count -eq 0 ];then
	${INSTALL_PATH}/bin/dal_set_agent_daemon.sh start
	if [ $? != 0 ];then
		echo ""
	fi	
	echo "[$(date +'%Y-%m-%d %H:%M:%S')]restart dal_set_agent_daemon" >> ${INSTALL_PATH}/log/auto_restart.log
fi
