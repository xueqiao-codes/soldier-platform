...
HashTblRemoveCallback global_hash_tbl_remove_callback = NULL;
...
static int hash_tbl_set_(qhasharr_t *tbl, const string &key, const string &val)
{
    ...
    while (!ret && errno == ENOBUFS) {
        ...
        if (removeRet == QCONF_OK) {
            if (global_hash_tbl_remove_callback) {
                global_hash_tbl_remove_callback(tbl, removeKey, true);
            }
            LRU::getInstance()->removeKey();
        }
        else {
            if (global_hash_tbl_remove_callback) {
                global_hash_tbl_remove_callback(tbl, removeKey, false);
            }
