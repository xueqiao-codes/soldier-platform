...
static void *assist_watcher_process(void *p)
{
    int interval = 180000 + rand() % 30000;
...
static void *msg_process(void *p)
{
    static int64_t RECEIVE_MSG_TOTAL_COUNT_ATTR_KEY = -1;
    if (RECEIVE_MSG_TOTAL_COUNT_ATTR_KEY == -1) {
        RECEIVE_MSG_TOTAL_COUNT_ATTR_KEY = soldier::attr::AttrReporterFactory::Global().thirtySecs().requireKey("qconf.agent.received.msg.total.count", {});
    }
    static int64_t RECEIVE_MSG_FAILED_COUNT_ATTR_KEY = -1;
    if (RECEIVE_MSG_FAILED_COUNT_ATTR_KEY == -1) {
        RECEIVE_MSG_FAILED_COUNT_ATTR_KEY = soldier::attr::AttrReporterFactory::Global().thirtySecs().requireKey(
            "qconf.agent.received.msg.failed.count", {});
    }
    ...
    while (!_stop_watcher_setting)
    {
        ...
        soldier::attr::AttrReporterFactory::Global().thirtySecs().inc(RECEIVE_MSG_TOTAL_COUNT_ATTR_KEY, 1);
        if (_stop_watcher_setting) break;
        if (QCONF_OK == ret)
        {
        ...
        else if (QCONF_ERR_MSGIDRM == ret)
        {
            ...
            soldier::attr::AttrReporterFactory::Global().thirtySecs().inc(RECEIVE_MSG_FAILED_COUNT_ATTR_KEY, 1);
...
static int64_t TRIGGER_SCRIPT_TOTAL_COUNT_KEY = -1;
static int64_t TRIGGER_SCRIPT_FAILED_COUNT_KEY = -1;
...
static void trigger_script(char data_type, const string &idc, const string &path, char trigger_type)
{
    ...
    if (TRIGGER_SCRIPT_TOTAL_COUNT_KEY == -1) {
        TRIGGER_SCRIPT_TOTAL_COUNT_KEY = soldier::attr::AttrReporterFactory::Global().thirtySecs().requireKey(
            "qconf.agent.trigger.script.total.count", {});
    }
    if (TRIGGER_SCRIPT_FAILED_COUNT_KEY == -1) {
        TRIGGER_SCRIPT_FAILED_COUNT_KEY = soldier::attr::AttrReporterFactory::Global().thirtySecs().requireKey(
            "qconf.agent.trigger.script.failed.count", {});
    }

    soldier::attr::AttrReporterFactory::Global().thirtySecs().inc(TRIGGER_SCRIPT_TOTAL_COUNT_KEY, 1);
    //execute script
    if (QCONF_OK != execute_script(head + content, _scexec_timeout))
    {
        soldier::attr::AttrReporterFactory::Global().thirtySecs().inc(TRIGGER_SCRIPT_FAILED_COUNT_KEY, 1);
