...
static int64_t LRU_CACHE_REMOVE_TOTAL_COUNT_ATTR_KEY = -1;
static int64_t LRU_CACHE_REMOVE_FAILED_COUNT_ATTR_KEY = -1;

void agent_on_hash_tbl_remove_callback(qhasharr_t* tbl, const std::string& removeKey, bool remove_success) {
    soldier::attr::AttrReporterFactory::Global().thirtySecs().inc(LRU_CACHE_REMOVE_TOTAL_COUNT_ATTR_KEY, 1);
    if (!remove_success) {
        soldier::attr::AttrReporterFactory::Global().thirtySecs().inc(LRU_CACHE_REMOVE_FAILED_COUNT_ATTR_KEY, 1);
    }
}
...
int main(int argc, char **arg)
{
    ...
    std::cout << "agent_dir:" << agent_dir << std::endl;
    ...
    std::cout << "check_pro_exist, pid_file="
              << pid_file << ", pid_fd=" << pid_fd
              << ", ret=" << ret << std::endl;
    ...
    if (QCONF_OK != ret)
    {
        std::cerr << "Failed to load configure! ret=" << ret << std::endl;
    ...
    std::cout << "starting daemon_mode=" << daemon_mode << std::endl;
    ...
    if (LRU_CACHE_REMOVE_TOTAL_COUNT_ATTR_KEY == -1) {
        LRU_CACHE_REMOVE_TOTAL_COUNT_ATTR_KEY = soldier::attr::AttrReporterFactory::Global().thirtySecs().requireKey(
                "qconf.agent.lru.cache.remove.total.count", {});
    }
    if (LRU_CACHE_REMOVE_FAILED_COUNT_ATTR_KEY == -1) {
        LRU_CACHE_REMOVE_FAILED_COUNT_ATTR_KEY = soldier::attr::AttrReporterFactory::Global().thirtySecs().requireKey(
                "qconf.agent.lru.cache.remove.failed.count", {});
    }

    global_hash_tbl_remove_callback = agent_on_hash_tbl_remove_callback;

    soldier::attr::AttrReporterFactory::Global().thirtySecs().keep(
            soldier::attr::AttrReporterFactory::Global().thirtySecs().requireKey("qconf.agent.keepalive", {}), 1);
