...
class CcTarget(Target):
    ...
    def _prebuilt_cc_library_pathname(self, dynamic=False):
        ...
        if dynamic:
            suffix = 'so'
        #print "_prebuilt_cc_library_pathname self.path=",self.path, options.m, options.profile
        #return os.path.join(self.path, 'lib%s_%s' % (options.m, options.profile),
        #                    'lib%s.%s' % (self.name, suffix))
        
        # find from search paths
        detect_file_path = os.path.join(self.path, 'lib%s_%s' % (options.m, options.profile),
                            'lib%s.%s' % (self.name, suffix))
        #print "detect_file_path=", detect_file_path
        if os.path.exists(detect_file_path):
          return detect_file_path     
        
        return os.path.join("cpp_platform/.libs", 'lib%s.%s' % (self.name, suffix))
    ...
    def _dynamic_deps_list(self):
        ...
        for lib in self.expanded_deps:
            ...
            #print "name=", self.name, "dep_target=", dep_target ,"dep_target.type", dep_target.type \
            #    ,", dep_target.name=", dep_target.name, "dep_target.srcs=",dep_target.srcs 
            ...
            #print  "_dynamic_deps_list lib_name=",lib_name       
            if lib_name:
                lib_list.append(lib_name)
    ...
    def _dynamic_cc_library(self):
        ...
        if self.srcs or self.expanded_deps:
            #print "self.expanded_deps=", self.expanded_deps
