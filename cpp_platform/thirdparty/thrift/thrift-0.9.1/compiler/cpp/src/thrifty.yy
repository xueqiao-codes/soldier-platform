...
Service:
  tok_service '(' tok_int_constant ')' tok_identifier Extends '{' FlagArgs FunctionList UnflagArgs '}' TypeAnnotations
    {
...
Function:
  CaptureDocText Oneway FunctionType FunctionName '(' FieldList ')' Throws TypeAnnotations CommaOrSemicolonOptional
    {
      $6->set_name(std::string($4->get_name()) + "_args");
      $$ = new t_function($3, $4->get_name(), $6, $8, $2);
      $$->set_number($4->get_number());
      if ($1 != NULL) {
        $$->set_doc($1);
      }
      if ($9 != NULL) {
...
FunctionName:
  tok_identifier
    {
		$$ = new t_function_name($1, -1);
		pwarning(1, "No unique number for function %s, This will cause some platform component not work",
      		$1);
	}
|
  tok_int_constant ':' tok_identifier
    {
    	if ($1 > 255 || $1 <= 0) {
    		yyerror("Function %s unique should be 0 < number <= 255 !", $3);
    	}
		$$ = new t_function_name($3, $1);
	}
