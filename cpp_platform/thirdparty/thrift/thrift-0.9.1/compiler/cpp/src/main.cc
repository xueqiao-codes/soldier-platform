...
t_generator_info g_generator_info;
...
bool gen_detect = false;

bool gen_analysis = false;
...
int main(int argc, char** argv) {
  ...
  if (generator_strings.empty() && !gen_detect && !gen_analysis) {
  ...
  if (gen_analysis) {
      ...
      for (std::vector<t_service*>::const_iterator it = program->get_services().begin();
                    it != program->get_services().end();
                    ++it) {
          std::cout << "{" << std::endl
                    << "  \"serviceName\" : \"" <<  (*it)->get_name() << "\"," << std::endl
                    << "  \"serviceKey\" : " << (*it)->get_servicekey() << "," << std::endl
                    << "  \"serviceNamespaces\" : {" << std::endl;

          size_t print_count = 0;
          for (std::map<std::string, std::string>::const_iterator namespace_it = (*it)->get_program()->get_namespaces().begin();
                  namespace_it != (*it)->get_program()->get_namespaces().end(); ++namespace_it, ++print_count) {

              std::cout << "    \"" << namespace_it->first << "\" : \"" << namespace_it->second << "\"";
              if (print_count + 1 < ((*it)->get_program()->get_namespaces()).size()) {
                  std::cout << ",";
              }
              std::cout << std::endl;
          ...
          std::cout << "  }," << std::endl
                    << "  \"serviceFunctions\" : [" << std::endl;

          print_count = 0;
          std::vector<t_function*>::const_iterator func_it = (*it)->get_functions().begin();
          for (;func_it != (*it)->get_functions().end(); ++func_it, ++print_count) {
              t_function* func = (*func_it);
              std::cout << "    {"  << std::endl;
              std::cout << "      \"functionName\" : \"" << func->get_name() << "\"," << std::endl
                        << "      \"functionKey\" : " << func->get_number() << std::endl;
              std::cout << "    }";

              if (print_count + 1 <  (*it)->get_functions().size()) {
                  std::cout << ",";
              }
              std::cout << std::endl;
          }
  ...
  if (gen_detect) {
	  // do not modify the output format, modified by wangli
      if (!program->get_namespace("py").empty()) {
          printf("\ndetect py namespace %s\n", program->get_namespace("py").c_str());
      }
      if (!program->get_namespace("cpp").empty()) {
          printf("\ndetect cpp namespace %s\n", program->get_namespace("cpp").c_str());
      }
      if (!program->get_namespace("java").empty()) {
          printf("\ndetect java namespace %s\n", program->get_namespace("java").c_str());
      }
