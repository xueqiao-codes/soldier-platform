...
class t_py_generator : public t_generator {
 public:
  ...
  void origin_generate_enum     (t_enum*     tenum);
  void generate_const    (t_const*    tconst);
  void origin_generate_const    (t_const*    tconst);

  void generate_struct   (t_struct*   tstruct);
  void origin_generate_struct(t_struct* tstruct);

  void generate_xception (t_struct*   txception);
  void origin_generate_xception(t_struct*   txception);

  void generate_service  (t_service*  tservice);
  void origin_generate_service(t_service* tservice);

  virtual void generate_project(t_service* tservice);
  void generate_stub_files(const std::string& package_dir, t_service* tservice);
 ...
 private:
  ...
  void open_ttypes_and_consts_file(bool is_client);
  void close_ttypes_and_consts_file(bool is_client);
  ...
  bool is_client_first_opened_ttypes_and_consts_;
};

std::string t_py_generator::init_client_source_dir() {
	string dir = get_real_py_module(program_, gen_twisted_);

	MKDIR(get_out_dir().c_str());
	client_source_dir_ = get_out_dir() +"py_clients";
	...
	while ((loc = dir.find(".")) != string::npos) {
		...
		std::ofstream init_py((subdir +"/__init__.py").c_str(), std::ios_base::app);
		init_py.close();
...
void t_py_generator::open_ttypes_and_consts_file(bool is_client) {
	string f_types_name = package_dir_+"/"+"ttypes.py";
	string f_consts_name = package_dir_+"/"+"constants.py";
	if (is_client) {
		if (!this->is_client_first_opened_ttypes_and_consts_) {
			...
			f_consts_ <<
			     ...
			     endl;
			this->is_client_first_opened_ttypes_and_consts_ = true;
		} else {
			f_types_.open(f_types_name.c_str(), std::ofstream::app);
			f_consts_.open(f_consts_name.c_str(), std::ofstream::app);
		}
	}
}

void t_py_generator::close_ttypes_and_consts_file(bool is_client) {
	f_types_.close();
	f_consts_.close();
}
...
void t_py_generator::init_generator() {
  ...
  is_client_first_opened_ttypes_and_consts_ = false;
  /*while (true) {
    // TODO: Do better error checking here.
    MKDIR(package_dir_.c_str());
    std::cout << "package_dir_=" << package_dir_ << std::endl;
...
void t_py_generator::generate_enum(t_enum* tenum) {
	package_dir_ = this->client_source_package_dir_;
	open_ttypes_and_consts_file(true);
	origin_generate_enum(tenum);
	close_ttypes_and_consts_file(true);
}
 ...
 */
void t_py_generator::origin_generate_enum(t_enum* tenum) {
 ...
 */
void t_py_generator::origin_generate_const(t_const* tconst) {
...
void t_py_generator::generate_struct(t_struct* tstruct) {
	package_dir_ = this->client_source_package_dir_;
	open_ttypes_and_consts_file(true);
	origin_generate_struct(tstruct);
	close_ttypes_and_consts_file(true);
}
 ...
 */
void t_py_generator::origin_generate_struct(t_struct* tstruct) {
...
void t_py_generator::generate_xception(t_struct* txception) {
	package_dir_ = this->client_source_package_dir_;
	open_ttypes_and_consts_file(true);
	origin_generate_xception(txception);
	close_ttypes_and_consts_file(true);
}
 ...
 */
void t_py_generator::origin_generate_xception(t_struct* txception) {
...
void t_py_generator::generate_service  (t_service*  tservice) {
	package_dir_ = this->client_source_package_dir_;
	origin_generate_service(tservice);
}
 ...
 */
void t_py_generator::origin_generate_service(t_service* tservice) {
  ...
  f_service_ << tservice->get_name() << "_SERVICE_KEY=" << tservice->get_servicekey() << endl;
...
void t_py_generator::generate_stub_files(const std::string& package_dir, t_service* tservice) {
	std::string stub_file = package_dir + "/client/" + tservice->get_name() + "Stub.py";
	ofstream f_stub_file;
	std::cout << "stub_file=" << stub_file.c_str() << endl;
	f_stub_file.open(stub_file.c_str());

	const vector<t_program*>& includes = program_->get_includes();

	f_stub_file << py_autogen_comment() << endl
				<< "import sys" << endl
				<< "import socket" << endl
				<< "from thrift import Thrift" << endl
				<< "from thrift.transport import TSocket" << endl
				<< "from thrift.transport import TTransport" << endl
				<< "from thrift.protocol import TCompactProtocol" << endl
				<< "from thrift.transport.TTransport import TTransportException" << endl;
	for (size_t i = 0; i < includes.size(); ++i) {
		f_stub_file << "from " <<  get_real_py_module(includes[i], gen_twisted_) << ".ttypes import *" << endl;
	}
	f_stub_file	<< "from " << module_ << ".ttypes import *" << endl;
	f_stub_file	<< "from " << module_ << " import " << tservice->get_name() << endl << endl
				<< "PYTHON_SUPPORT_DIR='/usr/local/soldier/route_agent'" << endl
				<< "if not (PYTHON_SUPPORT_DIR in sys.path):" << endl;
	indent_up();
	f_stub_file << indent() << "sys.path.append(PYTHON_SUPPORT_DIR)" << endl;
	indent_down();
	f_stub_file << "from route_finder_python import *" << endl << endl;

	f_stub_file << "class " << tservice->get_name() << "Stub:" << endl;
	indent_up();
	f_stub_file	<< indent() << "def __init__(self):" << endl;
	indent_up();
	f_stub_file << indent() << "self.__peerAddr = None" << endl << endl;
	indent_down();
	f_stub_file << indent() << "def setPeerAddr(self, peerAddr):" << endl;
	indent_up();
	f_stub_file << indent() << "self.__peerAddr = peerAddr" << endl << endl;
	indent_down();
	f_stub_file << indent() << "def __getServiceAddr(self, methodName='', routeKey=0):" << endl;
	indent_up();
	f_stub_file << indent() << "if self.__peerAddr != None and self.__peerAddr !='':" << endl;
	indent_up();
	f_stub_file << indent() << "return self.__peerAddr" << endl;
	indent_down();
	f_stub_file<< indent() << "return route_finder.GetRouteIp("
						   << tservice->get_name() << "." << tservice->get_name() << "_SERVICE_KEY"
						   << ", methodName, routeKey)"
						   << endl << endl;
	indent_down();
	...
	for(std::vector<t_function*>::const_iterator it = tservice->get_functions().begin();
		...
		f_stub_file << indent() << "def " << (*it)->get_name()
					<< "(self, routeKey, timeout, ";
		for (std::vector<t_field*>::const_iterator field_it = ++((*it)->get_arglist()->get_members().begin());
				field_it != (*it)->get_arglist()->get_members().end(); ++field_it) {
			f_stub_file << (*field_it)->get_name() << ",";
		}
		f_stub_file << "):" << endl ;
		indent_up();
		f_stub_file << indent() << "platformArgs=PlatformArgs()" << endl
					<< indent() << "frame = sys._getframe(1)" << endl
					<< indent() << "code = frame.f_code" << endl
					<< indent() << "platformArgs.sourceDesc = code.co_filename + '[' + code.co_name + \":\" + str(code.co_firstlineno) + ']'" << endl
					<< indent() << "platformArgs.sourceIp = socket.gethostbyname(socket.getfqdn())" << endl
					<< indent() << "platformArgs.remoteAddress = self.__getServiceAddr('" << (*it)->get_name() << "', routeKey)" << endl
					<< indent() << "if platformArgs.remoteAddress == None or platformArgs.remoteAddress == '':" << endl;
		indent_up();
		f_stub_file << indent() << "raise Exception('No RouteIp is Found')" << endl;
		indent_down();

		f_stub_file << indent() << "platformArgs.remotePort = 10000 + " << tservice->get_name() << "." << tservice->get_name() << "_SERVICE_KEY" << endl << endl
					<< indent() << "transport_socket = TSocket.TSocket(platformArgs.remoteAddress, platformArgs.remotePort)" << endl << endl
					<< indent() << "transport_socket.setTimeout(timeout)" << endl
					<< indent() << "transport = TTransport.TFramedTransport(transport_socket)" << endl
					<< indent() << "protocol = TCompactProtocol.TCompactProtocol(transport)" << endl
					<< indent() << "client=" << tservice->get_name() << ".Client(protocol)" << endl
					<< indent() << "try:" << endl;

		indent_up();
		f_stub_file << indent() << "transport.open()" << endl
					<< indent() << "result = client." << (*it)->get_name() << "(platformArgs,";
		for (std::vector<t_field*>::const_iterator field_it = ++((*it)->get_arglist()->get_members().begin());
						field_it != (*it)->get_arglist()->get_members().end(); ++field_it) {
			f_stub_file << (*field_it)->get_name() << ",";
		}
		f_stub_file << ")" << endl
					<< indent() << "route_finder.UpdateCallInfo("
					<< tservice->get_name() << "." << tservice->get_name()
					<< "_SERVICE_KEY, '" << (*it)->get_name() << "', platformArgs.remoteAddress, 0)" << endl
					<< indent() << "return result" << endl;
		indent_down();
		f_stub_file << indent() << "except TTransportException,t:" << endl;
		indent_up();
		f_stub_file	<< indent() << "route_finder.UpdateCallInfo("
					<< tservice->get_name() << "." << tservice->get_name()
					<< "_SERVICE_KEY, '" << (*it)->get_name() << "', platformArgs.remoteAddress, 1)" << endl
					<< indent() << "raise t" << endl;
		indent_down();
		f_stub_file << indent() << "finally:" << endl;
		indent_up();
		f_stub_file << indent() << "transport.close()" << endl << endl;
		indent_down();
...
void t_py_generator::generate_project(t_service* service) {
	open_ttypes_and_consts_file(true);
	close_ttypes_and_consts_file(true);

	// client
	generate_stub_files(client_source_package_dir_, service);

	std::ofstream init_py((package_dir_ + "/client/__init__.py").c_str());
	init_py << "__all__ = ['" << service->get_name() << "Stub']" << endl;
	init_py.close();
}
