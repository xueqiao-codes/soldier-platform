...
//    out_dir_base_ = (bean_style_ ? "gen-javabean" : "gen-java");
  ...
  void generate_consts_original(std::vector<t_const*> consts);
  ...
  void generate_enum_original(t_enum* tenum);

  void generate_struct  (t_struct*   tstruct);
  void generate_struct_original(t_struct* tstruct);

//  void generate_union   (t_struct*   tunion);
//  void generate_union_original(t_struct* tunion);

  void generate_xception(t_struct*   txception);
  void generate_xception_original(t_struct* txception);

  void generate_service (t_service*  tservice);
  void generate_service_original(t_service* tservice);
  
  // added by wileywang
  std::string init_server_project_dir();
  std::string init_client_source_dir();
  virtual void generate_project(t_service* tservice);
  void generate_comm_files(const std::string& package_dir, t_service* tservice);
  void generate_variable(std::ostream& out, t_service* tservice);
  void generate_adaptor(std::ostream& out, t_service* tservice);
  void generate_client_impl(std::ostream& out, t_service* tservice);
  void generate_server_impl (std::ostream& out, t_service* tservice);
  void generate_async_client_impl(std::ostream& out, t_service* tservice);
  void generate_function_mapping(std::ostream& out, t_service* tservice);
  void generate_pom(std::ostream& out, t_service* tservice);
  void generate_zip_description(t_service* tservice);
  void generate_configuration(t_service* tservice);
  // added end
  ...
  void generate_scheme_map(std::ostream& out, t_struct* tstruct);
  ...
  std::string function_signature(t_function* tfunction, std::string parameter1 = "", std::string prefix="", bool include_platformArgs = true);
  std::string function_signature_async(t_function* tfunction, bool use_base_method = false, std::string prefix="");
  
  std::string argument_list(t_struct* tstruct, bool include_types = true, bool include_platformArgs = true);
  std::string dot_argument_list(t_struct* tstruct,
		  	  	  	  	  	bool include_types = true,
		  	  	  	  	  	bool include_platformArgs = true);
  ...
  void generate_struct_desc(std::ostream& out, t_struct* tstruct);
  void generate_field_descs(std::ostream& out, t_struct* tstruct);
  void generate_field_name_constants(ofstream& out, t_struct* tstruct);
  
  void generate_import_container(std::ostream& out, t_service* t_service);
  void generate_import_none_base_types(std::ostream& out, t_service* t_service);
  void generate_log_function_params(const t_function* tfunction);
 ...
 private:
  ...
  std::string server_project_dir_;
  std::string client_source_dir_;
  std::string server_project_package_dir_;
  std::string client_source_package_dir_;
  // modified end
...
std::string t_java_generator::init_server_project_dir() {
	MKDIR(get_out_dir().c_str());
	server_project_dir_ = get_out_dir() + program_->get_name();

	package_name_ = program_->get_namespace("java");
	string dir = package_name_;

	string subdir = server_project_dir_;
	MKDIR(subdir.c_str());
	MKDIR((subdir + "/src").c_str());
	MKDIR((subdir + "/src/main").c_str());
	MKDIR((subdir + "/src/main/java").c_str());
	MKDIR((subdir + "/src/main/resources").c_str());
	MKDIR((subdir + "/src/main/config").c_str());

	subdir = subdir + "/src/main/java";
	...
	MKDIR((subdir + "/client").c_str());
	MKDIR((subdir + "/server").c_str());
	MKDIR((subdir + "/server/impl").c_str());

	return subdir;
}

std::string t_java_generator::init_client_source_dir() {
	MKDIR(get_out_dir().c_str());
	client_source_dir_ = get_out_dir() + program_->get_name() + "_client";

	package_name_ = program_->get_namespace("java");
	string dir = package_name_;

	string subdir = client_source_dir_;
	MKDIR(subdir.c_str());
...
//  MKDIR(server_project_dir_.c_str());
//  client_source_dir_ = get_out_dir() + program_->get_name() + "_client";
//  MKDIR(client_source_dir_.c_str());
//  package_name_ = program_->get_namespace("java");
//
//  string dir = package_name_;
//  string subdir = server_project_dir_;
...
//  MKDIR((subdir + "/server/impl").c_str());
  ...
  client_source_package_dir_ = init_client_source_dir();
  server_project_package_dir_ = init_server_project_dir();
}
...
void t_java_generator::generate_import_container(std::ostream& out, t_service* t_service)
{
	if(t_service->get_generator_info().hasList()){
		out << "import java.util.List;" << std::endl;
	}
	if(t_service->get_generator_info().hasMap()){
		out << "import java.util.Map;" << std::endl;
	}
	if(t_service->get_generator_info().hasSet()){
		out << "import java.util.Set;" << std::endl;
	}
	if(t_service->get_generator_info().hasBinary()){
		out << "import java.nio.ByteBuffer;" << std::endl;
	}
}

void t_java_generator::generate_import_none_base_types(std::ostream& out, t_service* t_service) {
	std::map<std::string, bool>::const_iterator it = t_service->get_generator_info().getNoneBaseTypes().begin();
	for(; it != t_service->get_generator_info().getNoneBaseTypes().end(); ++it) {
		out << "import " << this->package_name_ << "." << it->first << ";\n";
	}
}
...
void t_java_generator::generate_enum(t_enum* tenum) {
	package_dir_ = server_project_package_dir_;
	generate_enum_original(tenum);

	package_dir_ = client_source_package_dir_;
	generate_enum_original(tenum);
}

void t_java_generator::generate_enum_original(t_enum* tenum) {
  // Make output file
...
void t_java_generator::generate_consts(std::vector<t_const*> consts) {
	package_dir_ = server_project_package_dir_;
	generate_consts_original(consts);

	package_dir_ = client_source_package_dir_;
	generate_consts_original(consts);
}

void t_java_generator::generate_consts_original(std::vector<t_const*> consts) {
  if (consts.empty()) {
    return;
  }
...
void t_java_generator::generate_struct(t_struct* tstruct) {
	package_dir_ = server_project_package_dir_;
	generate_struct_original(tstruct);
	package_dir_ = client_source_package_dir_;
	generate_struct_original(tstruct);
}

void t_java_generator::generate_struct_original(t_struct* tstruct) {
  if (tstruct->is_union()) {
...
void t_java_generator::generate_xception(t_struct* txception) {
	package_dir_ = server_project_package_dir_;
	generate_xception_original(txception);
	package_dir_ = client_source_package_dir_;
	generate_xception_original(txception);
}

void t_java_generator::generate_xception_original(t_struct* txception) {
...
void t_java_generator::generate_service(t_service* tservice) {
	package_dir_ = server_project_package_dir_;
	generate_service_original(tservice);
	package_dir_ = client_source_package_dir_;
	generate_service_original(tservice);
}

void t_java_generator::generate_service_original(t_service* tservice) {
  // Make output file
...
void t_java_generator::generate_service_server(t_service* tservice) {
  ...
  // container will do this by svr_platform
...
void t_java_generator::generate_log_function_params(const t_function* tfunction)
{
	bool first = true;
	const std::vector<t_field*>& fields = tfunction->get_arglist()->get_members();
	indent(f_service_) << "sb.append(\"" << tfunction->get_name() << "\");" << endl;
	indent(f_service_) << "sb.append(\"(\");" << endl;
	for (std::vector<t_field*>::const_iterator f_iter = fields.begin();
		 f_iter != fields.end(); ++f_iter) {
	    if (first) {
	      first = false;
	    } else {
	      indent(f_service_) << "sb.append(\",\");" << endl;
	    }
	    indent(f_service_) << "sb.append(\"" << (*f_iter)->get_name() << "\");" << endl;
	    indent(f_service_) << "sb.append(\"=\");" << endl;
	    t_type* type = (*f_iter)->get_type();
	    if(type->is_base_type() && ((t_base_type*)type)->is_binary()) {
	    	indent(f_service_) << "org.apache.thrift.TBaseHelper.toString(args." << (*f_iter)->get_name() << ", sb);" << endl;
	    }
	    indent(f_service_) << "sb.append(args." << (*f_iter)->get_name() << ");" << endl;
	  }
	indent(f_service_)<< "sb.append(\")\");" << endl;
}
...
string t_java_generator::function_signature(t_function* tfunction,
											string parameter1,
                                            string prefix, 
											bool include_platformArgs) {
  t_type* ttype = tfunction->get_returntype();
  std::string result = type_name(ttype) + " " + prefix + tfunction->get_name() + "(";
  if(parameter1.length() > 0){
	  result += parameter1;
	  if (tfunction->get_arglist()->get_members().size() > 1 || include_platformArgs) {
		result += ", ";
	  }
  }
 
  result += argument_list(tfunction->get_arglist(), true, include_platformArgs) + ") throws ";
  t_struct* xs = tfunction->get_xceptions();
  const std::vector<t_field*>& xceptions = xs->get_members();
  vector<t_field*>::const_iterator x_iter;
  for (x_iter = xceptions.begin(); x_iter != xceptions.end(); ++x_iter) {
 ...
 */
string t_java_generator::argument_list(t_struct* tstruct, bool include_types, bool include_platformArgs) {
  string result = "";
...
void t_java_generator::generate_struct_desc(std::ostream& out, t_struct* tstruct) {
  indent(out) <<
...
void t_java_generator::generate_field_descs(std::ostream& out, t_struct* tstruct) {
  const vector<t_field*>& members = tstruct->get_members();
  vector<t_field*>::const_iterator m_iter;
...
void t_java_generator::generate_scheme_map(std::ostream& out, t_struct* tstruct) {
...
std::string t_java_generator::dot_argument_list(
				t_struct* arglist,
				bool include_types,
				bool include_platformArgs){
  std::string argstr = argument_list(arglist, include_types, include_platformArgs);
  if(argstr.length() > 0){
	return ", " + argstr;
  }
  return "";
...
// ===============================extension by svr_platform ====================================//
void t_java_generator::generate_variable(std::ostream& out, t_service* tservice) {
	std::string strVariableClass = tservice->get_name() + "Variable";
	out << "package " << this->package_name_ << ";\n\n"
		<< "public class " << strVariableClass << "{\n"
		<< "  public static final int serviceKey=" << tservice->get_servicekey() << ";\n"
		<< "}";
}

void t_java_generator::generate_adaptor(std::ostream& out, t_service* tservice){
	std::string strVariableClass = tservice->get_name() + "Variable";
	std::string strAdaptorClass = tservice->get_name() + "Adaptor";
	out << "package " << this->package_name_ << ".server;\n\n"
				  //<< "import java.io.File;\n"
	...
	generate_import_container(out, tservice);
	//if(!g_generator_info.hasMap()) {
		out << "import java.util.Map;" << std::endl;
	//}
	out << "import java.util.HashMap;" << std::endl;
	generate_import_none_base_types(out, tservice);
	out  << "import " << this->package_name_  << "." << tservice->get_name() << ";\n"
				  << "import " << this->package_name_ << "." << strVariableClass << ";\n"
				  //<< "import " << this->package_name_ << ".server.impl." << tservice->get_name() << "Handler;\n"
				  << "\n\n"
				  << "public abstract class " << strAdaptorClass << " implements " << tservice->get_name() << ".Iface{\n"
				  << "  // unmodified map, so we do not need lock for this " << std::endl
				  << "  private final Map<String, String[]> methodParameterNameMap = new HashMap<String, String[]>(); " << std::endl
				  << std::endl
				  << "  public String[] getMethodParameterName(String methodName) {" << std::endl
				  << "    return methodParameterNameMap.get(methodName);" << std::endl
				  << "  }" << std::endl << std::endl;
	for(std::vector<t_function*>::const_iterator funcIt = tservice->get_functions().begin();
			 funcIt != tservice->get_functions().end(); ++funcIt){
		out << "  @Override\n";
		out << "  public " << function_signature(const_cast<t_function*>(*funcIt)) << "{\n"
					 << "    TServiceCntl oCntl = new TServiceCntl("
					 <<  strVariableClass + ".serviceKey"
					 << ",\"" << (*funcIt)->get_name() << "\""
					 << "," << (*funcIt)->get_arglist()->get_members()[0]->get_name()
					 << ");" << std::endl;
		if( !(*funcIt)->get_returntype()->is_void()){
			out << "    return ";
		}
		out << (*funcIt)->get_name()
				     << "(oCntl" << dot_argument_list((*funcIt)->get_arglist(), false, false) << ");" << std::endl
					 << "  }\n" << std::endl;

		out << "  protected abstract "
					 << function_signature(const_cast<t_function*>(*funcIt), "TServiceCntl oCntl", "", false)
				     << ";\n\n";
	}

	out  << "  protected " << strAdaptorClass << "(){" << std::endl;
	// build parameter list
	for(std::vector<t_function*>::const_iterator funcIt = tservice->get_functions().begin();
				 funcIt != tservice->get_functions().end(); ++funcIt) {
		out << "    methodParameterNameMap.put(\"" << (*funcIt)->get_name() << "\",new String[]{";
		const std::vector<t_field*>& argsFields = (*funcIt)->get_arglist()->get_members();
		bool first = true;
		for(std::vector<t_field*>::const_iterator fieldIt = argsFields.begin();
				fieldIt != argsFields.end(); ++fieldIt) {
			if(!first){
				out<< ", ";
			}
			out << '"' << (*fieldIt)->get_name() << '"';
			first = false;
		}
		out << "});" << std::endl;
	}
	out  << "  }" << std::endl
				  ...
				  //<< "    if(args.length < 2){\n"
				  //<< "      System.err.println(\"LEAK OF CONFIG\");\n"
				  //<< "      System.exit(1);\n"
				  //<<  "    }\n"
				  //<< "    InputStream in = null;\n"
				  //<< "    try {\n"
				  ...
				  //<< "      props.load(in);\n"
				  //<< "    } catch (IOException e) {\n"
				  //<< "      System.err.println(\"LOAD CONFIG \" + args[0] + \" ERR, \" + e.getMessage());\n"
				  //<< "      System.exit(3);\n"
				  //<< "    }\n\n"
				  //<< "    //init Logger \n"
				  //<< "    LogManager.resetConfiguration();\n"
				  //<< "    PropertyConfigurator.configure(args[1]);\n"
				  //<< "    System.out.println(\"configure the log4j property \" + args[1]);\n\n"
				  //<< "    " << strAdaptorClass << " adaptor = new " << tservice->get_name()  << "Handler() ;\n"
				  //<< "    int err = adaptor.InitApp(props);\n"
				  //<< "    if(err != 0){\n"
				  //<< "      System.out.println(\"INIT APP FAILED \" + err);\n"
				  //<< "      System.exit(err);\n"
		          //<< "    }\n"
		          //<< "    int workerNum = Integer.parseInt(props.getProperty(\"workerNum\", \"100\"));\n"
		          //<< "    System.out.println(\"workernum=\" + workerNum);\n"
		          //<< "\n\n"
		          //<< "    // set Logger for Assert \n"
		 		  //<< "    Assert.SetLogger(Logger.getLogger(Assert.class));\n\n"
				  //<< "    String lpcModulesProperties = System.getenv(\"LPC_MODULES\");\n"
				  //<< "    if(lpcModulesProperties == null){\n"
				  //<< "      lpcModulesProperties = \"\";\n"
				  //<< "    }\n"
				  //<< "    String seperator = \":\";\n"
		          ...
		          //<< "    // run server\n"
		          //<< "    try {\n"
		          //<< "      int port = ServiceFinderFactory.getServiceFinder().getServicePort(" << strVariableClass << ".serviceKey);\n"
		          //<< "      TNonblockingServerSocket socket = new TNonblockingServerSocket(port);\n"
		          //<< "      final " << tservice->get_name()
		          //<< ".Processor<" << tservice->get_name() << ".Iface> processor = new "
		          //<<  tservice->get_name() << ".Processor<" << tservice->get_name() <<".Iface>(adaptor);\n"
		          //<< "      THsHaServer.Args serverArgs = new THsHaServer.Args(socket);\n"
		          //<< "      serverArgs.workerThreads(workerNum);\n"
...
void t_java_generator::generate_client_impl(std::ostream& out, t_service* tservice)
{
	std::string strClientClass = tservice->get_name() + "Stub";
	out     << "package " << this->package_name_ << ".client;\n\n"
			<< "import org.apache.thrift.TException;\n"
	...
	generate_import_none_base_types(out, tservice);
	out	 << "import " << this->package_name_ << "." << tservice->get_name() << ";\n"
				  << "import " << this->package_name_ << "." << tservice->get_name() << "Variable;\n\n"
				  << "public class " << strClientClass << " extends BaseStub {\n\n"
	              << "  public " << strClientClass << "() {\n"
	              << "    super(" << tservice->get_name() << "Variable.serviceKey);\n"
	              << "  }\n\n";
	for(std::vector<t_function*>::const_iterator funcIt = tservice->get_functions().begin();
			 funcIt != tservice->get_functions().end(); ++funcIt){
	    string arguments_withtypes = argument_list((*funcIt)->get_arglist(), true, false);
	    string arguments_notypes = argument_list((*funcIt)->get_arglist(), false, false);
	    std::string exceptions;
	    const std::vector<t_field*>& xceptions = (*funcIt)->get_xceptions()->get_members();
	    for (vector<t_field*>::const_iterator x_iter = xceptions.begin(); x_iter != xceptions.end(); ++x_iter) {
	        exceptions += type_name((*x_iter)->get_type(), false, false) + ", ";
	    }
	    exceptions += "TException";

	    out << "  public " << type_name((*funcIt)->get_returntype())
	                       << "  " << (*funcIt)->get_name() << "("
	                       << arguments_withtypes << ") throws " << exceptions << "{" << std::endl;
	    if ((*funcIt)->get_returntype()->is_void()) {
	        out << "    ";
	    } else {
	        out << "    return ";
	    ...
	    out << (*funcIt)->get_name() << "(" << argument_list((*funcIt)->get_arglist(), false, false) ;
	    if (arguments_notypes.length() > 0) {
	        out << ", ";
	    }
	    out << "new TStubOption().setCallTrace(Thread.currentThread().getStackTrace()[2])"
	        << ");" << std::endl;
	    out << "  }\n" << std::endl;
	    ...
	    out << "  public " << type_name((*funcIt)->get_returntype())
	                       << "  " << (*funcIt)->get_name() << "("
	                       << arguments_withtypes;
	    if (arguments_withtypes.length() > 0) {
	        out << ",";
	    }
	    out << "TStubOption platformStubOption) throws " << exceptions <<  " {" << std::endl;
	    out << "    InvokeMethodInfo invokeInfo = new InvokeMethodInfo(\""  << (*funcIt)->get_name()
	        << "\").setRouteKey(platformStubOption.getRouteKey()).setTimeoutMs(platformStubOption.getTimeoutMs());" << std::endl
	        << "    if (platformStubOption.getCallTrace() != null) { " << std::endl
	        << "      invokeInfo.setSourceCallTrace(platformStubOption.getCallTrace());" << std::endl
	        << "    } else {" << std::endl
	        << "      invokeInfo.setSourceCallTrace(Thread.currentThread().getStackTrace()[2]);" << std::endl
	        << "    }" << std::endl;
	    if ((*funcIt)->get_returntype()->is_void()) {
	        out << "    ";
	    } else {
	    ...
	    out << "super.runSync(new ThriftCallable<" << type_name((*funcIt)->get_returntype(), true) << ">(){" << std::endl
	        << "    @Override" << std::endl
	        << "    public " << type_name((*funcIt)->get_returntype(), true)
	        << " call(TProtocol protocol, PlatformArgs platformArgs) throws ErrorInfo, TException {" << std::endl;
	    if ((*funcIt)->get_returntype()->is_void()) {
	        out << "      new " <<  tservice->get_name() << ".Client(protocol)."
	                << (*funcIt)->get_name()
	                << "(platformArgs" << dot_argument_list((*funcIt)->get_arglist(), false, false) << ");\n";
	        out << "      return null;" << std::endl;
	    } else {
	        out << "      return new " <<  tservice->get_name() << ".Client(protocol)."
	            << (*funcIt)->get_name()
	            << "(platformArgs" << dot_argument_list((*funcIt)->get_arglist(), false, false) << ");\n";
	    }
		...
		out << "  public " <<type_name((*funcIt)->get_returntype())
					  << "  " << (*funcIt)->get_name() << "("
					  <<  "int routeKey, int timeout";
		if(arguments_withtypes.length() > 0){
			out << "," << arguments_withtypes;
		}
		out << ")" "throws " << exceptions << "{\n";
		if ((*funcIt)->get_returntype()->is_void()) {
		    out << "    ";
		} else {
		    out << "    return ";
		...
		out << "new TStubOption().setRouteKey(routeKey).setTimeoutMs(timeout).setCallTrace(Thread.currentThread().getStackTrace()[2])"
		        << ");" << std::endl;
		out << "  }\n" << std::endl;
	}
	out << "}\n";
...
void t_java_generator::generate_async_client_impl(std::ostream& out, t_service* service){
	std::string strAsyncClientName = service->get_name() + "AsyncStub";
	out << "package " << this->package_name_ << ".client" << ";\n\n"
					 << "import " << this->package_name_ <<  "." << service->get_name() << ";" << std::endl
					 << "import " << this->package_name_ <<  "." << service->get_name() << "Variable;" << std::endl
					 << "import org.apache.thrift.TException;" << std::endl
					 << "import java.net.InetAddress;\n"
					 << "import java.net.UnknownHostException;\n"
					 << "import org.soldier.base.logger.AppLog;\n"
					 << "import org.soldier.platform.svr_platform.client.AsyncCallRunner;" << std::endl
					 << "import org.soldier.platform.svr_platform.client.IMethodCallback;" << std::endl
					 << "import org.soldier.platform.svr_platform.client.SvrContainer;" << std::endl
					 << "import org.soldier.platform.svr_platform.client.TRequestOption;" << std::endl
					 << "import org.soldier.platform.svr_platform.client.TServiceCall;" << std::endl
					 << "import org.soldier.platform.svr_platform.comm.PlatformArgs;" << std::endl
					 << "import org.soldier.platform.svr_platform.client.BaseStub;" << std::endl;
	generate_import_container(out, service);
	generate_import_none_base_types(out, service);
	out << std::endl
		<< "public class " << strAsyncClientName << " extends BaseStub { \n"
		<< "  public " << strAsyncClientName << "() {" << std::endl
		<< "    super(" << service->get_name() << "Variable.serviceKey);" << std::endl
		<< "  }" << std::endl;

	for(std::vector<t_function*>::const_iterator functionIt = service->get_functions().begin();
			functionIt != service->get_functions().end(); ++functionIt){
		// oneway method has no async method
		if((*functionIt)->is_oneway()) {
			continue ;
		}
		
		std::string createFunctionServiceCall = "create_" + (*functionIt)->get_name() + "ServiceCall";
		std::string requestArg =  service->get_name() +  "." + (*functionIt)->get_name()+"_args";
		std::string responseResult = service->get_name() + "." + (*functionIt)->get_name()+"_result";
		out << "  public void send_" << (*functionIt)->get_name() << "(int routeKey, int timeout"
						 << dot_argument_list((*functionIt)->get_arglist(), true, false) << ") throws TException {" << std::endl
						 << "    PlatformArgs platformArgs = new PlatformArgs();" << std::endl
						 << "    platformArgs.setTimeoutMs(timeout);" << std::endl
						 << "    StackTraceElement callStackElement = Thread.currentThread().getStackTrace()[2];" << std::endl
						 << "    platformArgs.setSourceDesc(" << std::endl
						 << "        callStackElement.getClassName() + \"[\" + callStackElement.getMethodName() + \":\" " << std::endl
						 << "        + callStackElement.getLineNumber() + \"]\");" << std::endl
						 << "    try {" << std::endl
						 << "      platformArgs.setSourceIp(InetAddress.getLocalHost().getHostAddress());" << std::endl
						 << "    } catch (UnknownHostException e) {" << std::endl
						 << "      AppLog.w(e.getMessage(), e);" << std::endl
						 << "    }" << std::endl
						 << "    " << "SvrContainer.getInstance().sendRequest(" << std::endl
						 << "        " << createFunctionServiceCall
						 << "(routeKey, timeout, platformArgs" <<  dot_argument_list((*functionIt)->get_arglist(), false, false)
						 << "), new TRequestOption());" << std::endl
						 << "  }\n"
						 << std::endl;
		out << "  public void send_" << (*functionIt)->get_name() << "(int routeKey, int timeout"
						 << dot_argument_list((*functionIt)->get_arglist(), true, false) << ",TRequestOption requestOption) throws TException { " << std::endl
						 << "    PlatformArgs platformArgs = new PlatformArgs();" << std::endl
						 << "    platformArgs.setTimeoutMs(timeout);" << std::endl
						 << "    StackTraceElement callStackElement = Thread.currentThread().getStackTrace()[2];" << std::endl
						 << "    platformArgs.setSourceDesc(" << std::endl
						 ...
						 << "    " << "SvrContainer.getInstance().sendRequest(" << std::endl
						 << "        " << createFunctionServiceCall
						 << "(routeKey, timeout, platformArgs"
						 <<  dot_argument_list((*functionIt)->get_arglist(), false, false)
						 << "), requestOption);" << std::endl
						 << "  }\n" << std::endl;;
		out << "  public long " << (*functionIt)->get_name() << "(int routeKey, int timeout"
						 << dot_argument_list((*functionIt)->get_arglist(), true, false) << ", IMethodCallback<"
						 << requestArg << ", " << responseResult << "> callback) throws TException{" << std::endl
						 << "    PlatformArgs platformArgs = new PlatformArgs();" << std::endl
						 << "    platformArgs.setTimeoutMs(timeout);" << std::endl
						 << "    StackTraceElement callStackElement = Thread.currentThread().getStackTrace()[2];" << std::endl
						 << "    platformArgs.setSourceDesc(" << std::endl
						 ...
						 << "      AppLog.w(e.getMessage(), e);" << std::endl
						 << "    }" << std::endl
						 << "    return SvrContainer.getInstance().sendRequest(" << std::endl
						 << "            " << createFunctionServiceCall
						 << "(routeKey, timeout, platformArgs"
						 <<  dot_argument_list((*functionIt)->get_arglist(), false, false)
						 << "), callback);" << std::endl
						 << "  }\n" << std::endl;
		out << "  public long add_" << (*functionIt)->get_name() << "Call(AsyncCallRunner runner, int routeKey, int timeout"
						 << dot_argument_list((*functionIt)->get_arglist(), true, false) << ", IMethodCallback<"
						 << requestArg << ", " << responseResult << "> callback) throws TException{" << std::endl
						 << "    PlatformArgs platformArgs = new PlatformArgs();" << std::endl
						 << "    platformArgs.setTimeoutMs(timeout);" << std::endl
						 ...
						 << "      AppLog.w(e.getMessage(), e);" << std::endl
						 << "    }" << std::endl
						 << "    return runner.addAsyncCall(" << std::endl
						 << "            " << createFunctionServiceCall
						 << "(routeKey, timeout, platformArgs"
						 <<  dot_argument_list((*functionIt)->get_arglist(), false, false)
						 << "), callback);" << std::endl
						 << "  }\n" << std::endl;
		out << "  protected TServiceCall " << createFunctionServiceCall <<"(int routeKey, int timeout"
						 << dot_argument_list((*functionIt)->get_arglist(), true) << "){\n"
						 << "    " << "TServiceCall serviceCall = new TServiceCall();" << std::endl
						 << "    " << "if (!getPeerAddr().isEmpty()) {" << std::endl
						 << "    " << "  serviceCall.setChooseServiceIp(getPeerAddr());" << std::endl
						 << "    " << "}" << std::endl
						 << "    " << "if (getPeerPort() != 0) {" << std::endl
						 << "    " << "  serviceCall.setChooseServicePort(getPeerPort());" << std::endl
						 << "    " << "}" << std::endl
						 << "    " << "serviceCall.setServiceKey(" << service->get_name() << "Variable.serviceKey);" << std::endl
						 << "    " << "serviceCall.setRouteKey(routeKey);" << std::endl
						 << "    " << "serviceCall.setOneWay(" << ((*functionIt)->is_oneway() ? "true":"false") << ");" << std::endl
						 << "    " << requestArg << " request = new " << requestArg << "();" << std::endl;
		for(std::vector<t_field*>::const_iterator fieldIt = (*functionIt)->get_arglist()->get_members().begin();
				fieldIt != (*functionIt)->get_arglist()->get_members().end(); ++fieldIt) {
			out << "    " << "request.set" << get_cap_name((*fieldIt)->get_name())
							 << "(" << (*fieldIt)->get_name() << ");" << std::endl;
		}

		out << "    " << "serviceCall.setRequest(request);" << std::endl
						 << "    " << "serviceCall.setMethodName(\"" << (*functionIt)->get_name() << "\");" << std::endl
						 << "    " << "serviceCall.setTimeout(timeout);" << std::endl
						 << "    " << "serviceCall.setResponse(" << service->get_name() << "." << (*functionIt)->get_name() << "_result.class);" << std::endl
						 << "    " << "return serviceCall;" << std::endl
						 << "  }"
						 << std::endl
						 << std::endl;
	}
...
void t_java_generator::generate_server_impl (std::ostream& out, t_service* tservice)
{
	std::string strServerClass = tservice->get_name() + "Handler";
	out << "package " << this->package_name_ << ".server.impl" << ";\n\n"
			     << "\n";
	generate_import_container(out, tservice);
	
	out	 << "import java.util.Properties;\n"
				 << "import org.soldier.platform.svr_platform.container.TServiceCntl;\n"
				 << "import " << this->package_name_ << ".server." <<  tservice->get_name() << "Adaptor;\n";
	generate_import_none_base_types(out, tservice);
	out  << "\n"
			     << "public class " << strServerClass << " extends " << tservice->get_name() << "Adaptor"
			     << "{\n"
			     << "  @Override\n"
	...
	for(std::vector<t_function*>::const_iterator funcIt = tservice->get_functions().begin();
		 funcIt != tservice->get_functions().end(); ++funcIt){
		out << "  @Override\n";
		out << "  protected " << this->function_signature(const_cast<t_function*>(*funcIt), "TServiceCntl oCntl", "", false) << "{\n";
		t_type* returntype = (*funcIt)->get_returntype();
		if(!returntype->is_void()){
			out << "    return " ;
			if(returntype->is_base_type()){
				if(((t_base_type*)returntype)->is_binary()) {
					out << "null";
				} else if(returntype->is_string()){
					out << "\"\"";
				}else if(returntype->is_bool()){
					out << "false";
				}else{
					out << "0";
				}
...
void t_java_generator::generate_comm_files(const std::string& package_dir, t_service* tservice) {
	std::string strVariableClass = tservice->get_name() + "Variable";
	std::string strServiceVariablePath = package_dir + "/" + strVariableClass + ".java";
	std::ofstream oVariableFile;
	oVariableFile.open(strServiceVariablePath.c_str());
	generate_variable(oVariableFile, tservice);
	oVariableFile.close();

	std::ofstream oFunctionMappingFile(
			(package_dir + "/" + tservice->get_name() + "FunctionMapping.java").c_str());
	generate_function_mapping(oFunctionMappingFile, tservice);
	oFunctionMappingFile.close();

	std::string strClientClass = tservice->get_name() + "Stub";
	std::ofstream oClientFile;
	oClientFile.open((package_dir + "/client/" + strClientClass + ".java").c_str());
	generate_client_impl(oClientFile, tservice);
	oClientFile.close();

	std::string strAsyncClientName = tservice->get_name() + "AsyncStub";
	std::ofstream oAsyncClientFile;
	oAsyncClientFile.open((package_dir + "/client/"  + strAsyncClientName +  ".java").c_str());
	generate_async_client_impl(oAsyncClientFile, tservice);
	oAsyncClientFile.close();

}

void t_java_generator::generate_project(t_service* tservice) {
	// will generate this into server project dir
	generate_comm_files(server_project_package_dir_, tservice);

	std::string strAdaptorClass = tservice->get_name() + "Adaptor";
	std::string strAdaptorPath = server_project_package_dir_+ "/server/" + strAdaptorClass + ".java";
	std::ofstream oAdaptorFile(strAdaptorPath.c_str());
	generate_adaptor(oAdaptorFile, tservice);
	oAdaptorFile.close();
        
	std::string strServerClass = tservice->get_name() + "Handler";
	std::string outServerClassFileName = server_project_package_dir_ + "/server/impl/" + strServerClass + ".java";
	if (0 != access(outServerClassFileName.c_str(), 0)) {
		std::ofstream oServerFile(outServerClassFileName.c_str()) ;
		generate_server_impl(oServerFile, tservice);
		oServerFile.close();
	}

	
	std::string outPomFileName = server_project_dir_ + "/pom.xml";
	if (0 != access(outPomFileName.c_str(), 0)) {
		std::ofstream oPomFile(outPomFileName.c_str());
		generate_pom(oPomFile, tservice);
		oPomFile.close();
	}


	generate_configuration(tservice);
	generate_zip_description(tservice);

	// for client sources
	generate_comm_files(client_source_package_dir_, tservice);
}

void t_java_generator::generate_pom(std::ostream& out, t_service* tservice) {
	out << "<project xmlns=\"http://maven.apache.org/POM/4.0.0\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\""  << std::endl
			 << "xsi:schemaLocation=\"http://maven.apache.org/POM/4.0.0 http://maven.apache.org/xsd/maven-4.0.0.xsd\">" << std::endl
			 << "  <modelVersion>4.0.0</modelVersion>" << std::endl
			 << "  <groupId>" << package_name_ << "</groupId>" << std::endl
			 << "  <artifactId>" << program_->get_name() << "</artifactId>" << std::endl
			 << "  <version>1.0-SNAPSHOT</version>" << std::endl
			 << "  <packaging>jar</packaging>" << std::endl
			 << "  <name>" << program_->get_name() << "</name>" << std::endl << std::endl
			 << "  <dependencies>" << std::endl
			 << "    <dependency>" << std::endl
			 << "      <groupId>junit</groupId>" << std::endl
			 << "      <artifactId>junit</artifactId>" << std::endl
			 << "      <version>4.1</version>" << std::endl
	    	 << "      <scope>provided</scope>" << std::endl
	    	 << "    </dependency>" << std::endl
	    	 << "    <dependency>" << std::endl
			 << "      <groupId>org.soldier.platform</groupId>" << std::endl
	    	 << "      <artifactId>libsvr_platform</artifactId>" << std::endl
	    	 << "      <version>1.6</version>" << std::endl
	    	 << "    </dependency>" << std::endl
	    	 << "    <dependency>" << std::endl
			 << "      <groupId>com.antiy</groupId>" << std::endl
			 << "      <artifactId>error_code</artifactId>" << std::endl
			 << "      <version>0.1-SNAPSHOT</version>" << std::endl
			 << "    </dependency>" << std::endl
			 << "  </dependencies> " << std::endl << std::endl
			 << "  <build>" << std::endl
			 << "    <plugins>" << std::endl
			 << "      <plugin>" << std::endl
			 << "         <artifactId>maven-compiler-plugin</artifactId>" << std::endl
	         << "         <configuration>" << std::endl
	         << "           <source>1.8</source>" << std::endl
	         << "           <target>1.8</target>" << std::endl
			 << "           <compilerArgument>-Xlint:all</compilerArgument>" << std::endl
			 << "           <showWarnings>true</showWarnings>" << std::endl
			 << "           <encoding>utf-8</encoding>" << std::endl
			 << "           <showDeprecation>true</showDeprecation>" << std::endl
	         << "         </configuration>" << std::endl
	         << "      </plugin>" << std::endl << std::endl
	         << "      <plugin>" << std::endl
	    	 << "         <artifactId>maven-assembly-plugin</artifactId>" << std::endl
	    	 << "         <executions>" << std::endl
	         << "           <execution>" << std::endl
	         << "           <id>make-zip</id>" << std::endl
	         << "           <phase>package</phase>" << std::endl
	         << "           <goals>" << std::endl
	         << "              <goal>single</goal>" << std::endl
	       	 << "           </goals>" << std::endl
	         << "           <configuration>" << std::endl
	         << "             <descriptors>" << std::endl
	         << "                <descriptor>src/main/resources/zip.xml</descriptor>" << endl
	         << "             </descriptors>" << endl
	         << "           </configuration>" << endl
	         << "         </execution>" << endl
	         << "       </executions>" << endl
	         ...
	         << "  </build>" << endl << endl
	         // for inp
	         << "  <distributionManagement>" << endl
	         << "    <repository>" << endl
	         << "      <id>maven_server</id>" << endl
	         << "      <name>" << package_name_ << " Release Repository</name>" << endl
	         << "      <url>http://maven.xueqiao.cn/nexus/content/repositories/epower_release/</url>" << endl
	         << "    </repository>" << endl
	         << "    <snapshotRepository>" << endl
	         << "      <id>maven_server</id>" << endl
	         << "      <name>" << package_name_ << " Snapshot Repository</name>" << endl
	         << "     <url>http://maven.xueqiao.cn/nexus/content/repositories/epower_snapshots/</url>" << endl
	         << "    </snapshotRepository>" << endl
	         << "  </distributionManagement>" << endl << endl
	         << "</project>" << endl;
}

void t_java_generator::generate_zip_description(t_service* tservice) {
	std::ofstream oZipXmlFile((server_project_dir_ + "/src/main/resources/zip.xml").c_str());
	oZipXmlFile
		<< "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" << endl
		<< "<assembly" << endl
	    << "  xmlns=\"http://maven.apache.org/plugins/maven-assembly-plugin/assembly/1.1.0\"" << endl
	    << "  xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"" << endl
	    << "  xsi:schemaLocation=\"http://maven.apache.org/plugins/maven-assembly-plugin/assembly/1.1.0 http://maven.apache.org/xsd/assembly-1.1.0.xsd\">" << endl
	    << "  <formats>" << endl
	    << "    <format>zip</format>" << endl
	    << "  </formats>" << endl
	    << "  <baseDirectory>${artifactId}</baseDirectory>" << endl
	    << "  <fileSets>" <<endl
	    << "    <fileSet>" << endl
	    << "      <directory>${project.basedir}/src/main/config</directory>" << endl
	    << "      <outputDirectory>/</outputDirectory>" << endl
	    << "    </fileSet>" << endl
	    << "    <fileSet>" << endl
	    << "      <directory>${project.build.directory}</directory>" << endl
	    << "      <includes>" << endl
	    << "        <include>*.jar</include>" << endl
	    << "      </includes>" << endl
	    << "      <outputDirectory>/</outputDirectory>" << endl
	    << "    </fileSet>" << endl
	    << "  </fileSets>" << endl << endl
	    << "  <dependencySets>" << endl
	    << "    <dependencySet>" << endl
	    << "      <useProjectArtifact>false</useProjectArtifact>" << endl
	    << "      <outputDirectory>ext_jar</outputDirectory>" << endl
	    << "      <scope>runtime</scope>" << endl
	    << "    </dependencySet>" << endl
	    << "  </dependencySets>" << endl
	    << "</assembly>" << endl;
	oZipXmlFile.close();
...
void t_java_generator::generate_configuration(t_service* tservice) {
	std::ofstream oServicePropertiesFile((server_project_dir_ + "/src/main/config/service.properties").c_str());
	oServicePropertiesFile
		<< "# description of service" << endl
		<< "# you can add your configuration for service" << endl << endl
		<< "Adaptor-Class=" << package_name_ << ".server." << tservice->get_name() << "Adaptor" << endl;
	oServicePropertiesFile.close();

	std::ofstream oJvmConfigFile((server_project_dir_ + "/src/main/config/jvm.config").c_str());
	oJvmConfigFile << "\n";
	oJvmConfigFile.close();
}

void t_java_generator::generate_function_mapping(std::ostream& out, t_service* tservice) {
	std::string className = tservice->get_name() + "FunctionMapping";
	out << "package " << package_name_  << ";\n"
		...
		<< "  static {\n";
	for (std::vector<t_function*>::const_iterator it = tservice->get_functions().begin();
			it != tservice->get_functions().end(); ++it) {
		out << "    putEntry(\"" << (*it)->get_name() << "\"," << (*it)->get_number() << ");\n";
	}
	out	<< "  }\n\n"
