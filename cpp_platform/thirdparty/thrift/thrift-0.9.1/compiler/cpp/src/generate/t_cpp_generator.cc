 ...
 public:
  ...
  virtual std::string get_out_dir() const {
      if (use_include_prefix_) {
          // blade using include prefix, we need the same dir with origin
          return t_oop_generator::get_out_dir();
      }

      std::string out_base_dir;
      if (program_->is_out_path_absolute()) {
          out_base_dir = program_->get_out_path() + "/";
      } else {
          out_base_dir = program_->get_out_path() + out_dir_base_ + "/";
      }
      return out_base_dir + program_->get_name() + "/src/gen/";
  }

  virtual void generate_project(t_service* tservice);
  void generate_variables_h(std::ostream& out, t_service* tservice);
  void generate_variables_cpp(std::ostream& out, t_service* tservice);
  void generate_server(std::ostream& out, t_service* tservice);
  void generate_handler_h(std::ostream& out, t_service* tservice, const std::string& style);
  void generate_handler_cpp(std::ostream& out, t_service* tservice, const std::string& style);
  void generate_build_file(std::ostream& out, t_service* tservice);
  void generate_build_sh_file(std::ostream& out);
  void generate_blade_root_file(std::ostream& out);
  void generate_fabfile(std::ostream& out);
  void generate_supervisor_conf_file(std::ostream& out);
  void generate_gitignore(std::ostream& out);
  void generate_client_stub_h(std::ostream& out, t_service* tservice, const std::string& style);
  void generate_client_stub_cpp(std::ostream& out, t_service* tservice, const std::string& style);
  // added end
...
void t_cpp_generator::init_generator() {
  ...
  if (program_->is_out_path_absolute()) {
      output_base_dir_ = program_->get_out_path() + "/";
  } else {
      output_base_dir_ = program_->get_out_path() + out_dir_base_ + "/";
  }
  MKDIR(output_base_dir_.c_str());

  output_base_dir_ = output_base_dir_ + program_name_;
  MKDIR(output_base_dir_.c_str());
  MKDIR((output_base_dir_ + "/src").c_str());
  MKDIR((output_base_dir_ + "/src/gen").c_str());
  MKDIR((output_base_dir_ + "/src/gen/client").c_str());
  MKDIR((output_base_dir_ + "/src/impl").c_str());
...
void t_cpp_generator::generate_project(t_service* tservice) {
    std::ofstream variables_file_h;
    variables_file_h.open((output_base_dir_ + "/src/gen/" + tservice->get_name() + "_variables.h").c_str());
    generate_variables_h(variables_file_h, tservice);
    variables_file_h.close();

    std::ofstream variables_file_cpp;
    variables_file_cpp.open((output_base_dir_ + "/src/gen/" + tservice->get_name() + "_variables.cpp").c_str());
    generate_variables_cpp(variables_file_cpp, tservice);
    variables_file_cpp.close();

    std::ofstream stub_file_h;
    stub_file_h.open((output_base_dir_ + "/src/gen/client/" + program_name_ + "_stub.h").c_str());
    generate_client_stub_h(stub_file_h, tservice, "");
    stub_file_h.close();

    std::ofstream stub_file_cpp;
    stub_file_cpp.open((output_base_dir_ + "/src/gen/client/" + program_name_ + "_stub.cpp").c_str());
    generate_client_stub_cpp(stub_file_cpp, tservice, "");
    stub_file_cpp.close();

    std::ofstream server_file;
    server_file.open((output_base_dir_ + "/src/gen/" + tservice->get_name() + "_server.cpp").c_str());
    generate_server(server_file, tservice);
    server_file.close();

    std::ofstream handler_file_h;
    handler_file_h.open((output_base_dir_ + "/src/gen/" + tservice->get_name() + "_handler.h").c_str());
    generate_handler_h(handler_file_h, tservice, "");
    handler_file_h.close();

    std::ofstream handler_file_cpp;
    handler_file_cpp.open((output_base_dir_ + "/src/impl/" + tservice->get_name() + "_handler.cpp").c_str());
    generate_handler_cpp(handler_file_cpp, tservice, "");
    handler_file_cpp.close();

    std::ofstream build_file;
    build_file.open((output_base_dir_ + "/BUILD").c_str());
    generate_build_file(build_file, tservice);
    build_file.close();

    std::ofstream build_sh_file;
    build_sh_file.open((output_base_dir_ + "/build.sh").c_str());
    generate_build_sh_file(build_sh_file);
    build_sh_file.close();

    std::ofstream build_bladeroot_file;
    build_bladeroot_file.open((output_base_dir_ + "/BLADE_ROOT").c_str());
    generate_blade_root_file(build_bladeroot_file);
    build_bladeroot_file.close();

    std::ofstream build_fabfile_file;
    build_fabfile_file.open((output_base_dir_ + "/fabfile.py").c_str());
    generate_fabfile(build_fabfile_file);
    build_fabfile_file.close();

    std::ofstream build_supervisor_conf_file;
    build_supervisor_conf_file.open((output_base_dir_ + "/supervisor.conf").c_str());
    generate_supervisor_conf_file(build_supervisor_conf_file);
    build_supervisor_conf_file.close();

    std::ofstream build_gitignore_file;
    build_gitignore_file.open((output_base_dir_ + "/.gitignore").c_str());
    generate_gitignore(build_gitignore_file);
    build_gitignore_file.close();
}

void t_cpp_generator::generate_variables_h(std::ostream& out, t_service* tservice) {
    out << "#ifndef " << tservice->get_name() << "_VARIABLES_H" << std::endl;
    out << "#define " << tservice->get_name() << "_VARIABLES_H" << std::endl;
    out << ns_open_ << std::endl;
    out << "extern int " + tservice->get_name() << "_service_key;" << std::endl;
    out << ns_close_ << std::endl;
    out << "#endif " << std::endl;
}

void t_cpp_generator::generate_variables_cpp(std::ostream& out, t_service* tservice) {
    out << ns_open_ << std::endl;
    out << "int " << tservice->get_name() << "_service_key = " << tservice->get_servicekey() << ";" << std::endl;
    out << ns_close_ << std::endl;
}

void t_cpp_generator::generate_server(std::ostream& out, t_service* tservice) {
    out << "#include \"base/app_log.h\"" << std::endl
        << "#include \"" << tservice->get_name() << "_variables.h\"" << std::endl
        << "#include \"thrift/concurrency/PosixThreadFactory.h\"" << std::endl
        << "#include \"thrift/protocol/TCompactProtocol.h\"" << std::endl
        << "#include \"thrift/server/TNonblockingServer.h\"" << std::endl
        << "#include \"thrift/transport/TServerSocket.h\"" << std::endl
        << "#include \"thrift/TProcessor.h\"" << std::endl
        << "#include \"" << tservice->get_name() << "_handler.h\"" << std::endl
        <<  std::endl;
    ...
    indent_up();
    out << indent() << "soldier::base::AppLog::Init(\"service/" << program_name_ << "\");" << std::endl
        << indent() << "boost::shared_ptr<apache::thrift::TProcessor> processor(" << std::endl;
    indent_up();
    out << indent() << "new " << ns << tservice->get_name() << "Processor(boost::shared_ptr<"
        << ns << tservice->get_name() << "If>(new " << ns << tservice->get_name() << "Handler())));" << std::endl;
    indent_down();
    out << indent() << "boost::shared_ptr<apache::thrift::protocol::TProtocolFactory>" << std::endl;
    indent_up();
    out << indent() << "protocolFactory(new apache::thrift::protocol::TCompactProtocolFactory());" << std::endl;
    indent_down();
    out << indent() << "boost::shared_ptr<apache::thrift::concurrency::ThreadManager> threadManager" << std::endl;
    indent_up();
    out << indent() << "= apache::thrift::concurrency::ThreadManager::newSimpleThreadManager(20);" << std::endl;
    indent_down();
    out << indent() << "boost::shared_ptr<apache::thrift::concurrency::PosixThreadFactory>" << std::endl;
    indent_up();
    out << indent() << "threadFactory(new ::apache::thrift::concurrency::PosixThreadFactory());" << std::endl;
    indent_down();

    out << indent() << "threadManager->threadFactory(threadFactory); " << std::endl;
    out << indent() << "threadManager->start();" << std::endl;

    out << indent() << "int port = 10000 + " << ns << tservice->get_name() << "_service_key;" << std::endl;
    out << indent() << "apache::thrift::server::TNonblockingServer server(processor, protocolFactory, port, threadManager);" << std::endl;
    out << indent() << "server.serve();" << std::endl;
    out << indent() << "return 0;" << std::endl;
    indent_down();
    out << "}" << std::endl;
}

void t_cpp_generator::generate_client_stub_h(std::ostream& out, t_service* tservice, const std::string& style) {
	out << "#ifndef " << tservice->get_name() << "_STUB_H" << std::endl;
	out << "#define " << tservice->get_name() << "_STUB_H" << std::endl;
	out << "#include \"stub_base.h\"" << std::endl;
	out << "#include \"" << program_name_ << "_types.h\"" << std::endl;
	out << ns_open_ << std::endl;

	out << "class " << tservice->get_name()
		<< "Stub : public ::soldier::svr_platform::TStubBase {" << std::endl;
	out << "public:" << std::endl;

	indent_up();
	out << indent() << tservice->get_name() << "Stub();" << std::endl
		<< indent() << "virtual ~" << tservice->get_name() << "Stub() = default;" << std::endl
		<< std::endl;

	vector<t_function*> functions = tservice->get_functions();
	vector<t_function*>::iterator func_iter;
	for (func_iter = functions.begin(); func_iter != functions.end(); ++func_iter) {
		t_type* ttype = (*func_iter)->get_returntype();
		if (is_complex_type(ttype)) {
			out << indent() << "void ";
		} else {
			out << indent() << type_name(ttype) << " ";
		}

		out << (*func_iter)->get_name() << "(" << std::endl
			<< indent() << indent() << "const ::soldier::svr_platform::TPrepareSyncCallArgs& platformCallArgs" << std::endl;

		if (is_complex_type(ttype)) {
			out << indent() << indent() << ", " << type_name(ttype) << "& _return" << std::endl;
		}

		std::string arguments = argument_list((*func_iter)->get_arglist(), true, true, false);
		if (!arguments.empty()) {
			out	<< indent() << indent() << arguments << std::endl;
		}
		out << indent() << indent() << ") throw(::platform::comm::ErrorInfo, ::apache::thrift::TException);" << std::endl;
	}
...
void t_cpp_generator::generate_client_stub_cpp(std::ostream& out, t_service* tservice, const std::string& style) {
	out << "#include \"" << program_name_ << "_stub.h\"" << std::endl
		<< "#include \"" << tservice->get_name() << ".h\"" << std::endl
		<< "#include \"" << tservice->get_name() << "_variables.h\"" << std::endl
		<< "using namespace soldier::svr_platform;" << std::endl << std::endl;

	out << ns_open_ << std::endl;

	out << tservice->get_name() << "Stub::" << tservice->get_name() << "Stub()" << std::endl
		<< "  :TStubBase(" << tservice->get_name() << "_service_key) {" << std::endl
		<< "}" << std::endl;
	...
	for (func_iter = functions.begin(); func_iter != functions.end(); ++func_iter) {
		...
		indent_up();
		out << tservice->get_name() << "Stub::" << (*func_iter)->get_name() << "(" << std::endl
			<< indent() << "const TPrepareSyncCallArgs& platformCallArgs" << std::endl;

		if (is_complex_type(ttype)) {
			out << indent() << ", " << type_name(ttype) << "& _return" << std::endl;
		}
		std::string arguments = argument_list((*func_iter)->get_arglist(), true, true, false);
		if (!arguments.empty()) {
		...
		out << indent() << ") throw(::platform::comm::ErrorInfo, ::apache::thrift::TException) {" << std::endl;

		out << indent() << "TPrepareSyncCallResp callResp;" << std::endl
			<< indent() << "prepareSyncCall(platformCallArgs, callResp);" << std::endl
			<< indent() << tservice->get_name() << "Client client(callResp.protocol);" << std::endl;

		if (ttype->is_void() || is_complex_type(ttype)) {
			out << indent();
		}  else {
			out << indent() << "return ";
		}

		out << "client." << (*func_iter)->get_name() << "(";

		if (is_complex_type(ttype)) {
			out << "_return, callResp.platform_args";
		} else {
			out << "callResp.platform_args";
		}

		t_struct* args = (*func_iter)->get_arglist();
		vector<t_field*>::const_iterator args_field_iter;

		for (args_field_iter = args->get_members().begin()
				; args_field_iter != args->get_members().end()
				; ++args_field_iter) {
			if((*args_field_iter)->get_type()->get_name() == "PlatformArgs"){
				t_program* program = (*args_field_iter)->get_type()->get_program();
				if(program != NULL && program->get_name() == "comm"){
					continue;
				}
			}
			out << ", " << (*args_field_iter)->get_name();
		}
	...
	out << std::endl << ns_close_ << std::endl;
}

void t_cpp_generator::generate_handler_h(std::ostream& out, t_service* tservice, const std::string& style) {
    out << "#ifndef " << tservice->get_name() << "_HANDLER_H" << std::endl;
    out << "#define " << tservice->get_name() << "_HANDLER_H" << std::endl;
    out << "#include \"" << tservice->get_name() << ".h\"" << std::endl;
    out << ns_open_ << std::endl;

    out << "class " <<  tservice->get_name() << "Handler : public " << tservice->get_name() << "If {" << std::endl;
    out << "public:" << std::endl;
    indent_up();
    out << indent() << tservice->get_name() << "Handler();" << std::endl
        << indent() << "virtual ~" << tservice->get_name() << "Handler();" << std::endl;

    vector<t_function*> functions = tservice->get_functions();
    vector<t_function*>::iterator f_iter;
    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
        out << indent() << "virtual " << function_signature(*f_iter, style) << ";" << endl;
    }
...
void t_cpp_generator::generate_handler_cpp(std::ostream& out
        , t_service* tservice
        , const std::string& style) {
    std::string handler_class_name =  tservice->get_name() + "Handler";

    out << "#include \"" << tservice->get_name() << "_handler.h\"" << std::endl
        << "#include \"base/app_log.h\"" << std::endl << std::endl;

    out << ns_open_ << std::endl << std::endl;

    out << handler_class_name << "::" << handler_class_name << "() {" << std::endl;
    indent_up();
    out << indent() << "APPLOG_INFO(\"" << handler_class_name << " Init Begin...\");" <<  std::endl;
    out << indent() << "// Add Init Code Here..." << std::endl;
    out << indent() << "APPLOG_INFO(\"" << handler_class_name << " Init End...\");" << std::endl;
    indent_down();
    out << "}" << std::endl << std::endl;
    ...
    vector<t_function*>::iterator f_iter;
    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
        out << function_signature(*f_iter, style, handler_class_name + "::" ) << " { " << std::endl;
...
void t_cpp_generator::generate_build_file(std::ostream& out, t_service* tservice) {
    out << "cc_binary(" << std::endl
        << "    name = '" << program_name_ << "'," << std::endl
        << "    srcs = [" << std::endl
        << "        'src/gen/" << program_name_ << "_constants.cpp'," << std::endl
        << "        'src/gen/" << program_name_ << "_types.cpp'," << std::endl
        << "        'src/gen/" << tservice->get_name() << ".cpp'," << std::endl
        << "        'src/gen/" << tservice->get_name() << "_server.cpp'," << std::endl
        << "        'src/gen/" << tservice->get_name() << "_variables.cpp'," << std::endl
        << "        'src/impl/" << tservice->get_name() << "_handler.cpp'," << std::endl
        << "    ]," << std::endl
        << "    incs = [" << std::endl
        << "        'src/gen'," << std::endl
        << "        'src/impl'," << std::endl
        << "    ]," << std::endl
        << "    deps = [" << std::endl
        << "        '//cpp_platform/svr:svrcpp'," << std::endl
        << "    ]" << std::endl
        << ")" << std::endl;
}

void t_cpp_generator::generate_build_sh_file(std::ostream& out) {
    out <<
"#!/bin/bash"  << std::endl <<
"#" << std::endl <<
"" << std::endl <<
"if [ -L \"$0\" ];then" << std::endl <<
"    THIS_SCRIPT_REAL_DIR=$( dirname $0 )" << std::endl <<
"else" << std::endl <<
"    # run this script directly, can be run everywhere" << std::endl <<
"    THIS_SCRIPT_REAL_DIR=\"$(dirname \"${BASH_SOURCE:-$0}\")\"" << std::endl <<
"    if [ \"$( echo ${THIS_SCRIPT_REAL_DIR} | grep '^/' | wc -l )\" = \"0\" ];then" << std::endl <<
"        THIS_SCRIPT_REAL_DIR=\"$( pwd )/${THIS_SCRIPT_REAL_DIR}\"" << std::endl <<
"    fi" << std::endl <<
"fi" << std::endl <<
"cd ${THIS_SCRIPT_REAL_DIR} > /dev/null" << std::endl <<
"CURRENT_SCRIPT_DIR=$( pwd )" << std::endl <<
"cd - > /dev/null" << std::endl <<
"" << std::endl <<
"CPP_PLATFORM_DIR=\"/data/code/soldier/platform/cpp_platform\"" << std::endl <<
"if [ ! -f \"${CURRENT_SCRIPT_DIR}/cpp_platform\" ];then" << std::endl <<
"    ln -sf $CPP_PLATFORM_DIR cpp_platform" << std::endl <<
"fi" << std::endl <<
"" << std::endl <<
"pushd ${CURRENT_SCRIPT_DIR} > /dev/null" << std::endl <<
"source cpp_platform/envsetup.sh" << std::endl <<
"blade build" << std::endl <<
"popd > /dev/null" << std::endl;
}

void t_cpp_generator::generate_blade_root_file(std::ostream& out) {
    out <<
"cc_config(" << std::endl <<
"    extra_incs=[" << std::endl <<
"        'cpp_platform/thirdparty'," << std::endl <<
"        'cpp_platform/.libs/thrift/include'" << std::endl <<
"    ]," << std::endl <<
"    cxxflags=['-std=c++11'" << std::endl <<
"        , '-D__LINUX__=1'" << std::endl <<
"        , '-DRAPIDJSON_HAS_STDSTRING=1']," << std::endl <<
"    warnings = [" << std::endl <<
"        '-Wall'," << std::endl <<
"        '-Wextra'," << std::endl <<
"        # disable some warnings enabled by Wextra" << std::endl <<
"        '-Wno-unused-but-set-variable'," << std::endl <<
"        '-Wno-unused-parameter'," << std::endl <<
"        '-Wno-unused-local-typedefs'," << std::endl <<
"        '-Wno-missing-field-initializers'," << std::endl <<
"        # other useful warnings" << std::endl <<
"        '-Wendif-labels'," << std::endl <<
"        '-Wfloat-equal'," << std::endl <<
"        '-Wformat=2'," << std::endl <<
"        '-Wframe-larger-than=69632', # A 64k buffer and other small vars" << std::endl <<
"        '-Wmissing-include-dirs'," << std::endl <<
"        '-Wpointer-arith'," << std::endl <<
"        '-Wwrite-strings'," << std::endl <<
"        # error flags" << std::endl <<
"        '-Werror=char-subscripts'," << std::endl <<
"        '-Werror=comments'," << std::endl <<
"        '-Werror=conversion-null'," << std::endl <<
"        '-Werror=empty-body'," << std::endl <<
"        '-Werror=endif-labels'," << std::endl <<
"        '-Werror=format'," << std::endl <<
"        #'-Werror=format-nonliteral'," << std::endl <<
"        '-Werror=missing-include-dirs'," << std::endl <<
"        '-Werror=overflow'," << std::endl <<
"        '-Werror=parentheses'," << std::endl <<
"        '-Werror=reorder'," << std::endl <<
"        '-Werror=return-type'," << std::endl <<
"        '-Werror=sequence-point'," << std::endl <<
"        '-Werror=sign-compare'," << std::endl <<
"        '-Werror=switch'," << std::endl <<
"        '-Werror=type-limits'," << std::endl <<
"        '-Werror=uninitialized'," << std::endl <<
"        # Masked it at first" << std::endl <<
"        # '-Werror=unused-function'," << std::endl <<
"        '-Werror=unused-label'," << std::endl <<
"        '-Werror=unused-result'," << std::endl <<
"        '-Werror=unused-value'," << std::endl <<
"        '-Werror=unused-variable'," << std::endl <<
"        '-Werror=write-strings'," << std::endl <<
"    ]," << std::endl <<
"    # C++ only warning flags" << std::endl <<
"    cxx_warnings = [" << std::endl <<
"        '-Wno-invalid-offsetof'," << std::endl <<
"        '-Wnon-virtual-dtor'," << std::endl <<
"        '-Woverloaded-virtual'," << std::endl <<
"        '-Wvla'," << std::endl <<
"        #'-Werror=non-virtual-dtor'," << std::endl <<
"        '-Werror=overloaded-virtual'," << std::endl <<
"        '-Werror=vla'," << std::endl <<
"    ]," << std::endl <<
"    # C only warning flags" << std::endl <<
"    c_warnings = ['-Werror-implicit-function-declaration']," << std::endl <<
"    cpplint = 'cpplint.py', # Default is unpathed cpplint.py command" << std::endl <<
"    optimize = ['-O2']," << std::endl <<
"    benchmark_libs=['//toft/base:benchmark']," << std::endl <<
"    benchmark_main_libs=['//toft/base:benchmark_main']," << std::endl <<
"    linkflags = [" << std::endl <<
"        '-fPIC'" << std::endl <<
"        ,  '-lpthread'" << std::endl <<
"        , '-L/usr/local/soldier/qconf_agent/lib/'" << std::endl <<
"        , '-lqconf'" << std::endl <<
"        , '-Wl,-rpath=/usr/local/soldier/qconf_agent/lib'" << std::endl <<
"        , '-L/usr/local/soldier/route_agent/lib/'" << std::endl <<
"        , '-lroute_agent_shared'" << std::endl <<
"        , '-Wl,-rpath=/usr/local/soldier/route_agent/lib'" << std::endl <<
"    ]" << std::endl <<
")" << std::endl <<
"" << std::endl <<
"cc_library_config(" << std::endl <<
"    generate_dynamic = False," << std::endl <<
")" << std::endl <<
"" << std::endl <<
"thrift_library_config(" << std::endl <<
"    # The path to thrift compiler." << std::endl <<
"    thrift='thrift'," << std::endl <<
"    # The libraries depended by thrift." << std::endl <<
"    thrift_libs=['//cpp_platform/thirdparty/thrift:thrift']," << std::endl <<
"    # The extra include paths to be passed to thrift compiler." << std::endl <<
"    thrift_incs=['//thrift_include']" << std::endl <<
")" << std::endl <<
"" << std::endl;

}

void t_cpp_generator::generate_fabfile(std::ostream& out) {
    out <<
"#" << std::endl <<
"# install script" << std::endl <<
"#" << std::endl <<
"" << std::endl <<
"import sys" << std::endl <<
"sys.path.append(\"/public/scripts/machine\")" << std::endl <<
"" << std::endl <<
"import os" << std::endl <<
"" << std::endl <<
"from fabric.contrib import project, console, files" << std::endl <<
"from common import getEnv" << std::endl <<
"from supervisord import *" << std::endl <<
"from epower import *" << std::endl <<
"" << std::endl <<
"app_name=\"" << program_name_ << "\"" << std::endl <<
"build_dir=\"build64_release\"" << std::endl <<
"install_dir=\"/usr/local/soldier/apps/\"+ app_name" << std::endl <<
"" << std::endl <<
"def _analysis_dependencies(build_target_dir):" << std::endl <<
"    return local(\"ldd \" + build_target_dir + app_name + \" | grep \" +  build_dir + \" | awk '{print $1}'\", capture=True).split()" << std::endl <<
"" << std::endl <<
"def _install(env, fabfile_dir):" << std::endl <<
"    build_target_dir= fabfile_dir + \"/\" + build_dir + \"/\";" << std::endl <<
"    run(\"mkdir -p \" + install_dir )" << std::endl <<
"    if not files.exists(\"/data/applog/service/\" + app_name):" << std::endl <<
"        run(\"mkdir -p /data/applog/service/\" + app_name)" << std::endl <<
"" << std::endl <<
"    for dependency in _analysis_dependencies(build_target_dir):" << std::endl <<
"        dependency_dir = os.path.dirname(install_dir + \"/\" + dependency)" << std::endl <<
"        if not files.exists(dependency_dir):" << std::endl <<
"            run(\"mkdir -p \" + dependency_dir)" << std::endl <<
"        run('cp -f ' + fabfile_dir + '/' + dependency + ' ' + dependency_dir)" << std::endl <<
"" << std::endl <<
"    run('cp -f ' + build_target_dir + app_name + \" \" + install_dir)" << std::endl <<
"    run('cp -f ' + fabfile_dir + \"/supervisor.conf \" + install_dir)" << std::endl <<
"    addSupervisorConfig(install_dir)" << std::endl <<
"    run(\"supervisorctl update\")" << std::endl <<
"    restart()" << std::endl <<
"" << std::endl <<
"" << std::endl <<
"def install():" << std::endl <<
"    _install(getEnv(), os.path.dirname(__file__))" << std::endl <<
"" << std::endl <<
"def restart():" << std::endl <<
"    run(\"supervisorctl restart \" + app_name)" << std::endl <<
"" << std::endl <<
"def uninstall():" << std::endl <<
"    removeSupervisorConfig(install_dir)" << std::endl <<
"    run(\"supervisorctl update\")" << std::endl <<
"    run(\"rm -rf \" + install_dir)" << std::endl
<< std::endl;

}

void t_cpp_generator::generate_supervisor_conf_file(std::ostream& out) {
    out <<
"[program:" << program_name_ << "]" << std::endl <<
"directory = /usr/local/soldier/apps/" << program_name_ << std::endl <<
"command = /usr/local/soldier/apps/" << program_name_ << "/" << program_name_ << std::endl <<
"autostart = true" << std::endl <<
"startsecs = 5" << std::endl <<
"autorestart = true" << std::endl <<
"startretries = 3" << std::endl <<
"user = epower" << std::endl <<
"redirect_stderr = true" << std::endl <<
"stdout_logfile_maxbytes = 10MB" << std::endl <<
"stdout_logfile_backups = 20" << std::endl <<
"stdout_logfile = /data/applog/service/" << program_name_ << "/stdout.log" << std::endl << std::endl;
}

void t_cpp_generator::generate_gitignore(std::ostream& out) {
    out <<
"/out" << std::endl <<
"*.pyc" << std::endl <<
"*.class" << std::endl <<
...
".project" << std::endl <<
".cproject" << std::endl <<
".pydevproject" << std::endl <<
".classpath" << std::endl <<
".Building.lock" << std::endl <<
".sconsign.dblite" << std::endl <<
"blade_bin/" << std::endl <<
"build64_release/" << std::endl <<
"target/" << std::endl;
}
  ...
  if (style == "") {
    if (is_complex_type(ttype)) {
      return
        ...
        argument_list(arglist, name_params, true, include_platformArgs) + ")";
    } else {
      return
        type_name(ttype) + " " + prefix + tfunction->get_name() +
        "(" + argument_list(arglist, name_params, false, include_platformArgs) + ")";
    }
