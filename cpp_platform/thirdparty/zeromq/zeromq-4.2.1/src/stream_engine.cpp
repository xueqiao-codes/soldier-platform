...
int zmq::stream_engine_t::process_heartbeat_message(msg_t * msg_)
{
    if(memcmp(msg_->data(), "\4PING", 5) == 0) {
        ...
        int remote_heartbeat_ttl_real = int(remote_heartbeat_ttl) * 100;

        if(!has_ttl_timer && remote_heartbeat_ttl_real > 0) {
            add_timer(remote_heartbeat_ttl_real, heartbeat_ttl_timer_id);
            has_ttl_timer = true;
        }
