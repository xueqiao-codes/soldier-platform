...
public class FileStorageInfoDaoAsyncStub { 
  ...
  public void send_updateStorage(int routeKey, int timeout, StorageInfo storageInfo) throws TException {
    PlatformArgs platformArgs = new PlatformArgs();
    StackTraceElement callStackElement = Thread.currentThread().getStackTrace()[2];
    platformArgs.setSourceDesc(
        callStackElement.getClassName() + "[" + callStackElement.getMethodName() + ":" 
        + callStackElement.getLineNumber() + "]");
    SvrContainer.getInstance().sendRequest(
        create_updateStorageServiceCall(routeKey, timeout, platformArgs, storageInfo), new TRequestOption());
  }

  public void send_updateStorage(int routeKey, int timeout, StorageInfo storageInfo,TRequestOption requestOption) throws TException { 
    PlatformArgs platformArgs = new PlatformArgs();
    StackTraceElement callStackElement = Thread.currentThread().getStackTrace()[2];
    platformArgs.setSourceDesc(
        callStackElement.getClassName() + "[" + callStackElement.getMethodName() + ":" 
        + callStackElement.getLineNumber() + "]");
    SvrContainer.getInstance().sendRequest(
        create_updateStorageServiceCall(routeKey, timeout, platformArgs, storageInfo), requestOption);
  }

  public long updateStorage(int routeKey, int timeout, StorageInfo storageInfo, IMethodCallback<FileStorageInfoDao.updateStorage_args, FileStorageInfoDao.updateStorage_result> callback) throws TException{
    PlatformArgs platformArgs = new PlatformArgs();
    StackTraceElement callStackElement = Thread.currentThread().getStackTrace()[2];
    platformArgs.setSourceDesc(
        callStackElement.getClassName() + "[" + callStackElement.getMethodName() + ":"
        + callStackElement.getLineNumber() + "]");
    return SvrContainer.getInstance().sendRequest(
            create_updateStorageServiceCall(routeKey, timeout, platformArgs, storageInfo), callback);
  }

  public long add_updateStorageCall(AsyncCallRunner runner, int routeKey, int timeout, StorageInfo storageInfo, IMethodCallback<FileStorageInfoDao.updateStorage_args, FileStorageInfoDao.updateStorage_result> callback) throws TException{
    PlatformArgs platformArgs = new PlatformArgs();
    StackTraceElement callStackElement = Thread.currentThread().getStackTrace()[2];
    platformArgs.setSourceDesc(
        callStackElement.getClassName() + "[" + callStackElement.getMethodName() + ":" 
  ...
  protected TServiceCall create_updateStorageServiceCall(int routeKey, int timeout, org.soldier.platform.svr_platform.comm.PlatformArgs platformArgs, StorageInfo storageInfo){
    TServiceCall serviceCall = new TServiceCall();
    serviceCall.setServiceKey(FileStorageInfoDaoVariable.serviceKey);
    serviceCall.setRouteKey(routeKey);
    serviceCall.setOneWay(false);
    FileStorageInfoDao.updateStorage_args request = new FileStorageInfoDao.updateStorage_args();
    request.setPlatformArgs(platformArgs);
    request.setStorageInfo(storageInfo);
    serviceCall.setRequest(request);
    serviceCall.setMethodName("updateStorage");
    serviceCall.setTimeout(timeout);
    serviceCall.setResponse(FileStorageInfoDao.updateStorage_result.class);
    return serviceCall;
  }
